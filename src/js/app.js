...
function seleccionarFecha() {
    ...
    inputFecha.addEventListener('input', e => {
        ...
        if([6, 0].includes(dia)) {
            e.target.value = '';
            mostrarAlerta('Fines de semana no operamos', 'error', '#paso-2');
        } else {
...
function seleccionarHora() {
    ...
    inputHora.addEventListener('input', function(e) {
        ...
        if(hora <= 12 || hora >= 21) {
            e.target.value = '';
            mostrarAlerta('Hora no Válida', 'error', '#paso-2');
        } else {
...
function mostrarAlerta(mensaje, tipo, elemento, desaparece = true) {
    ...
    referencia.appendChild(alerta, referencia);
...
async function reservarCita() {
    ...
    try {
        ...
        if(resultado.resultado) {
            Swal.fire({
                title: "Cita Creada",
                text: "Tu cita fue agregada correctamente!",
                icon: "success",
                customClass: {
